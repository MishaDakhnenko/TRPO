...
void outputInformationAboutTicketSelling(struct ticketselling *a, int *size)
{
	for(int i=0;i<*size;i++)
	{
	printf("%d.|Number of flight: &2d | Airplane type: %s | Destination: %s | Departure date: %2d | Departure time: %2d | Arrival time: %2d | Airplane capacity: %3d | Number of remaining business class tickets: %3d | Price of business class ticket: %2d | Number of remaining economy class tickets: %3d | Price of economy class ticket: %2d\n", i+1, (a + i)->numberofflight,(a + i)->airplanetype,(a + i)->destination,(a + i)->departuredate,(a + i)->departuretime,(a + i)->arrivaltime,(a + i)->airplanecapacity,(a + i)->numberofremainingbusinessclasstickets,(a + i)->priceofbusinessclassticket,(a + i)->numberofremainingeconomyclasstickets,(a + i)->priceofeconomyclassticket);
	}
	...
	while (m==0)
{
		...
		switch(n)
	{
		...
		case 3:
		    ...
		    scanf("%2s %2s", &(a+n1-1)->destination);
		    break;
		case 4:
...
void addNewTicketSelling(struct ticketselling *a, int *size)
{
	...
	scanf("%2d %2d", &(a+ *size)->airplanetype);
	printf("Enter Destination: ");
	scanf("%2s %2s", &(a+*size)->destination);
